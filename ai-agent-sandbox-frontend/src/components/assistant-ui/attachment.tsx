...
const useAttachmentSrc = () => {
  const { file, src: remoteImageSrc } = useAssistantState(
    useShallow(({ attachment }): { file?: File; src?: string } => {
      ...
      const imageContentSrc = attachment.content?.find(
        (content) => content.type === "image"
      )?.image;
      if (!imageContentSrc) {
        return {};
      }
      return { src: imageContentSrc };
    })
  );

  return useFileSrc(file) ?? remoteImageSrc;
};
  ...
  return (
    <Avatar className="aui-attachment-tile-avatar h-full w-full rounded-none">
      <AvatarImage
        ...
        src={src}
      />
      <AvatarFallback delayMs={isImage ? AVATAR_IMAGE_FALLBACK_DELAY_MS : 0}>
